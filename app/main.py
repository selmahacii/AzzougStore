...
def run_db_migrations():
    ...
    statements = [
        ...
        "WITH ranked_orders AS (SELECT id, FIRST_VALUE(id) OVER (PARTITION BY COALESCE(store_id, 'default'), LOWER(TRIM(customer_phone)) ORDER BY created_at ASC) as parent_id FROM orders WHERE customer_phone IS NOT NULL AND TRIM(customer_phone) != '' AND LOWER(TRIM(customer_phone)) != 'inconnu' AND (is_deleted IS FALSE OR is_deleted IS NULL) AND status != 'MERGED') UPDATE orders o SET status = 'MERGED', parent_order_id = r.parent_id, is_deleted = TRUE FROM ranked_orders r WHERE o.id = r.id AND o.id != r.parent_id",
