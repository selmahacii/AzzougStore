...
def run_db_migrations():
    ...
    statements = [
        ...
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS payday INTEGER",
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS last_salary_paid_at TIMESTAMP WITHOUT TIME ZONE",
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS last_salary_paid_month VARCHAR",
        ...
        "UPDATE orders SET status = 'ABANDONED', is_abandoned_cart = TRUE WHERE id LIKE 'cd0cdcbe%' OR store_sequence_number = 768 OR customer_phone = '0658225644'",
        ...
        "UPDATE financial_transactions ft SET amount = GREATEST(0, (o.total - COALESCE(o.delivery_fee, 0))) FROM orders o WHERE ft.reference = 'COD-' || o.order_number AND ft.amount != GREATEST(0, (o.total - COALESCE(o.delivery_fee, 0)))",
        "WITH wallet_sums AS (SELECT wallet_id, COALESCE(SUM(CASE WHEN type = 'PAYMENT' THEN amount ELSE 0 END), 0) AS calc_in, COALESCE(SUM(CASE WHEN type IN ('DISBURSEMENT', 'CHARGE') THEN amount ELSE 0 END), 0) AS calc_out FROM financial_transactions GROUP BY wallet_id) UPDATE wallets w SET total_in = ws.calc_in, total_out = ws.calc_out, balance = (ws.calc_in - ws.calc_out) FROM wallet_sums ws WHERE w.id = ws.wallet_id",
    ]
...
def create_initial_superadmin():
    ...
    try:
        ...
        # Auto-heal recent checkout orders 830-835 to normal orders & enqueue missing Meta CAPI events
        db.execute(text("""
            UPDATE orders
            SET is_abandoned_cart = FALSE,
                is_duplicate = FALSE,
                status = CASE WHEN status = 'MERGED' OR status = 'ABANDONED' THEN 'ASSIGNED' ELSE status END,
                order_number = REPLACE(order_number, 'ABN-', 'ORD-')
            WHERE (store_sequence_number IN (830, 831, 832, 833, 834, 835) OR customer_phone IN ('0658075922', '0780125700'))
