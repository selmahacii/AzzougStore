...
def get_transaction_order_details(
    ...
    from app.models.marketing import MetaAdsCampaign, MetaAdsConfig
    ...
    if is_marketing:
        ...
        if "Campagne:" in desc:
            ...
            camp_name = after_camp.splitlines()[0].strip()
        ...
        for o in attributed_orders:
            if o.landing_url:
                landing_pages_set.add(o.landing_url)
            elif o.referrer:
                landing_pages_set.add(o.referrer)

        if not landing_pages_set:
            if campaign and campaign.product_id:
                p = db.query(Product).filter(Product.id == campaign.product_id).first()
                if p:
                    landing_pages_set.add(f"https://azzougstore.com/products/{p.slug or p.id}")
            else:
                landing_pages_set.add("https://azzougstore.com/ (Page d'accueil & Boutique)")
        ...
        for o in attributed_orders:
            for item in o.items:
                if item.product_id and item.product_id not in seen_prod_ids:
                    seen_prod_ids.add(item.product_id)
                    ...
                    targeted_products.append({
                        ...
                        "category": p.category if p else "Catalogue"
                    })
        ...
        marketing_data = {
            ...
            "date_end": date_end,
            "landing_pages": list(landing_pages_set),
