    ...
    if store_id:
        ...
        query = query.filter(
            or_(
                ...
                cast(User.assigned_store_ids, String).ilike(f"%{store_id}%")
            )
        )
    if role:
...
def get_user_performance(
    ...
    audit_logs = (
        ...
        .filter(and_(OrderEvent.actor_id == user_id, Order.store_id == store_id))
    ...
    return {
        ...
        "stats": {
            ...
            # commission_per_order: per-order rate for PER_DELIVERED_ORDER employees, None for MONTHLY_SALARY
