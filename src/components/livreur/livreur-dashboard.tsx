 ...
 * the parcels explicitly assigned to them. Three sections: deliveries,
 * product management and inventory (both store-scoped on the backend).
 */
export default function LivreurDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-24">
      ...
      {section === 'products'   && <LivreurProductView />}
      {section === 'inventory'  && <LivreurStockView />}
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════════════════════════
   LivreurProductView — read-only product catalog (no cost prices)
   ═══════════════════════════════════════════════════════════════════════════ */

function LivreurProductView() {
  ...
  const productsQuery = useQuery<any>({
    queryKey: ['livreur-products', activeStore?.id],
    queryFn: () => apiFetch(`/api/v1/products?store_id=${activeStore?.id}&pageSize=200`),
  ...
  const activeProductIds = useMemo(() => {
    ...
    const active = orders.filter(o => ['CONFIRMED', 'SHIPPED'].includes(o.status));
    return new Set(active.flatMap(o => (o.items ?? []).map((i: any) => i.product_id)));
  }, [ordersQuery.data]);

  const allProducts: any[] = productsQuery.data?.data ?? [];
  const filtered = allProducts.filter(p =>
    !search.trim() || p.name?.toLowerCase().includes(search.toLowerCase())
  );
  const sorted = [...filtered].sort((a, b) =>
    (activeProductIds.has(b.id) ? 1 : 0) - (activeProductIds.has(a.id) ? 1 : 0)
  );
    ...
    <div className="max-w-3xl mx-auto p-4 space-y-4">
      ...
      {productsQuery.isLoading ? (
        <div className="flex items-center justify-center py-16">
          <Loader2 className="size-7 animate-spin text-slate-200" />
        </div>
      ) : sorted.length === 0 ? (
        <div className="bg-white border border-dashed rounded-3xl py-16 text-center">
          <Boxes className="size-9 text-slate-200 mx-auto" />
          <p className="mt-3 text-xs font-bold text-slate-300 uppercase tracking-widest">Aucun produit</p>
        </div>
      ) : (
        <div className="space-y-2.5">
          {sorted.map((p: any) => {
            const isActive = activeProductIds.has(p.id);
            ...
            return (
              <div
                key={p.id}
                className={cn(
                  'bg-white border rounded-2xl p-3.5 flex items-center gap-3',
                  isActive && 'border-cyan-200 bg-cyan-50/30'
                )}
              >
                {p.main_image ? (
                  <img src={p.main_image} alt={p.name} className="size-14 rounded-xl object-cover shrink-0 border border-slate-100" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {isActive && (
                      <span className="text-[9px] font-black text-cyan-600 bg-cyan-100 rounded-full px-1.5 py-0.5 uppercase shrink-0">
                        En livraison
                      </span>
                    )}
                    <p className="text-sm font-black text-slate-800 leading-tight truncate">{p.name}</p>
                  </div>
                  <p className="text-sm font-bold text-cyan-700 mt-0.5">{formatPrice(p.price)}</p>
                  {p.variants?.length > 0 && (
                ...
                <div className="text-right shrink-0">
                  <p className={cn(
                    ...
                    available <= 0 ? 'text-rose-500' : isLow ? 'text-amber-500' : 'text-emerald-600'
                  )}>
                    {available}
                  </p>
...
/* ═══════════════════════════════════════════════════════════════════════════
   LivreurStockView — simplified stock overview (no cost prices, no admin controls)
   Highlights products from active deliveries and shows low-stock alerts.
   ═══════════════════════════════════════════════════════════════════════════ */

function LivreurStockView() {
  const { activeStore, user } = useAppStore();

  const summaryQuery = useQuery<any>({
    queryKey: ['inventory', 'summary', activeStore?.id],
    queryFn: () => apiFetch(`/api/v1/stock/summary?store_id=${activeStore?.id}`),
    enabled: !!activeStore?.id,
  ...
  const alertsQuery = useQuery<any>({
    queryKey: ['stock-alerts', activeStore?.id],
    queryFn: () => apiFetch(`/api/v1/stock/alerts?store_id=${activeStore?.id}`),
    enabled: !!activeStore?.id,
  ...
  const alerts: any[] = alertsQuery.data?.data ?? [];
  ...
  const activeDeliveryProducts = products.filter(p => activeProductIds.has(p.id));
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-white border rounded-2xl p-3 text-center">
          <p className="text-[8px] font-black uppercase tracking-wider text-slate-400 leading-tight">Produits actifs</p>
          <p className="text-2xl font-black text-slate-800 tabular-nums mt-1">{summary.totalProducts ?? '—'}</p>
        </div>
        <div className={cn('border rounded-2xl p-3 text-center', (summary.lowStockCount ?? 0) > 0 ? 'bg-amber-50 border-amber-100' : 'bg-white')}>
          <p className="text-[8px] font-black uppercase tracking-wider text-amber-500 leading-tight">Stock faible</p>
          <p className="text-2xl font-black text-amber-500 tabular-nums mt-1">{summary.lowStockCount ?? '—'}</p>
        </div>
        <div className={cn('border rounded-2xl p-3 text-center', (summary.outOfStockCount ?? 0) > 0 ? 'bg-rose-50 border-rose-100' : 'bg-white')}>
          <p className="text-[8px] font-black uppercase tracking-wider text-rose-500 leading-tight">Rupture</p>
          <p className="text-2xl font-black text-rose-500 tabular-nums mt-1">{summary.outOfStockCount ?? '—'}</p>
        </div>
      </div>

      {/* Active delivery products */}
      {activeDeliveryProducts.length > 0 && (
        <div>
          <p className="text-[10px] font-black uppercase tracking-wider text-cyan-600 mb-2">
            Produits de mes livraisons en cours
          </p>
          <div className="space-y-2">
            {activeDeliveryProducts.map((p: any) => {
              const available = Math.max(0, (p.stock ?? 0) - (p.reserved_stock ?? 0));
              return (
                <div key={p.id} className="bg-cyan-50 border border-cyan-200 rounded-2xl p-3.5 flex items-center gap-3">
                  {p.main_image ? (
                    <img src={p.main_image} alt="" className="size-10 rounded-xl object-cover border border-cyan-100 shrink-0" />
                  ) : (
                    <div className="size-10 rounded-xl bg-cyan-100 flex items-center justify-center shrink-0">
                      <Package className="size-4 text-cyan-500" />
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-black text-slate-800 truncate">{p.name}</p>
                    {p.sku && <p className="text-[10px] text-slate-400 font-bold">{p.sku}</p>}
                  </div>
                  <div className="text-right shrink-0">
                    <p className={cn(
                      'text-lg font-black tabular-nums',
                      available <= 0 ? 'text-rose-500' : available <= (p.low_stock_threshold ?? 5) ? 'text-amber-500' : 'text-emerald-600'
                    )}>
                      {available}
                    </p>
      ...
      {alerts.length > 0 && (
        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 space-y-2">
          <p className="text-[10px] font-black uppercase tracking-wider text-amber-600">⚠️ Alertes stock</p>
          {alerts.map((a: any) => (
            <div key={a.id} className="flex items-center justify-between py-1.5 border-b border-amber-100 last:border-0 gap-3">
              <div className="flex items-center gap-2 min-w-0">
                {a.main_image ? (
                  <img src={a.main_image} className="size-7 rounded-lg object-cover border border-amber-100 shrink-0" alt="" />
                ) : (
                ...
                <p className="text-sm font-bold text-amber-800 truncate">{a.name}</p>
              </div>
              <span className={cn(
                'text-xs font-black px-2 py-0.5 rounded-full shrink-0',
                a.stock <= 0 ? 'bg-rose-100 text-rose-600' : 'bg-amber-100 text-amber-700'
              )}>
      ...
      {/* All products stock */}
      <div>
        <p className="text-[10px] font-black uppercase tracking-wider text-slate-400 mb-2">Tous les produits</p>
        {productsQuery.isLoading ? (
          ...
          <div className="space-y-2">
            {products.map((p: any) => {
              const available = Math.max(0, (p.stock ?? 0) - (p.reserved_stock ?? 0));
              const isActive = activeProductIds.has(p.id);
              const isLow = available > 0 && available <= (p.low_stock_threshold ?? 5);
              return (
                <div
                  key={p.id}
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      {isActive && <span className="size-1.5 rounded-full bg-cyan-500 shrink-0" />}
                      <p className="text-sm font-bold text-slate-700 truncate">{p.name}</p>
                    </div>
                    {p.sku && <p className="text-[10px] text-slate-400 mt-0.5">{p.sku}</p>}
                  </div>
                  <span className={cn(
                    'text-sm font-black tabular-nums shrink-0 px-3 py-1 rounded-xl',
                    available <= 0
                    ...
                    {available <= 0 ? 'Rupture' : `${available} unités`}
                  </span>
                </div>
              );
            })}
