...
const MOVEMENT_TYPE_LABELS: Record<string, string> = {
   ...
   ORDER_CONFIRM: 'Vente confirmée',
   ORDER_RESERVE: 'Réservation',
   ORDER_RELEASE: 'Libération réservation',
   RETURN_RESTOCK: 'Retour réintégré',
...
export default function StockTracker() {
  ...
  const { data: movementsData, isLoading: isLoadingMovements } = useQuery({
    queryKey: ['stock-movements', activeStore?.id, search, movementType, dateFrom, dateTo, page],
    queryFn: () => {
      ...
      return apiFetch<{ success: boolean; data: any[]; total: number; totalPages: number }>(`/api/v1/stock/?${params.toString()}`);
    },
    enabled: !!activeStore?.id,
  });

  React.useEffect(() => { setPage(1); }, [search, movementType, dateFrom, dateTo]);
  ...
  const handleExport = () => {
    ...
    const csv = [
      ['Date', 'Produit', 'Quantité', 'Type', 'Batch', 'Motif', 'Éditeur'],
      ...events.map((e: any) => [
        new Date(e.created_at).toLocaleString(),
        e.product_name || e.product_id,
        e.quantity,
        MOVEMENT_TYPE_LABELS[e.type] || e.type,
        e.batch_id || '',
        e.reason || '',
        e.actor?.name || 'Système',
      ]),
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-2xl border p-4 flex flex-col md:flex-row gap-4 items-center shadow-sm" style={{ borderColor: C.border }}>
         ...
         <div className="flex gap-3 w-full md:w-auto">
            ...
            <Button onClick={handleExport} disabled={events.length === 0} variant="outline" className="h-12 px-6 rounded-xl border-[#E9ECF0] text-xs font-bold bg-white text-[#636E72] hover:bg-[#F8F9FC] disabled:opacity-40">
         ...
         >
            <div className="space-y-2">
               ...
               <div className="grid grid-cols-2 gap-2 text-xs">
                  <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="h-10 bg-[#F8F9FC] border-[#E9ECF0] rounded-lg font-bold" />
                  <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="h-10 bg-[#F8F9FC] border-[#E9ECF0] rounded-lg font-bold" />
               </div>
            </div>
            <div className="space-y-2">
            ...
            {(search || movementType !== 'all' || dateFrom || dateTo) && (
               <div className="flex items-end">
                  <button
                     onClick={() => { setSearch(''); setMovementType('all'); setDateFrom(''); setDateTo(''); }}
                     className="h-10 px-4 rounded-lg text-xs font-bold text-rose-500 hover:bg-rose-50 transition-colors"
                  >
      ...
      <div className="bg-white rounded-2xl border border-[#E9ECF0] overflow-hidden shadow-sm">
                     ...
                     <tr key={i} className="hover:bg-[#FAFBFD] transition-colors group">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                           <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col">
                                 <span className="text-sm font-bold text-[#2D3436] tracking-tight">{e.product_name || `Réf. ${e.product_id?.slice(0, 8)}`}</span>
                                 {e.order_number && (
                                    <span className="text-[10px] font-bold text-[#6C5CE7] uppercase font-mono mt-0.5">Cmd #{e.order_number}</span>
                                 )}
                              </div>
                           </div>
                        </td>
                        <td className="px-4 py-4 text-center">
                           <span className={cn(
                              ...
                              e.quantity >= 0 ? "bg-[#E6FFF8] border-[#D1F9ED] text-[#00B894]" : "bg-[#FFEDE9] border-[#FFDED6] text-[#E17055]"
                           )}>{e.quantity >= 0 ? '+' : ''}{e.quantity}</span>
                        </td>
                        <td className="px-6 py-4 text-center">
                           <div className="flex flex-col items-center">
                              <Badge className={cn(
                                 "border-none rounded-md px-2.5 py-1 text-[10px] font-black tracking-widest uppercase flex items-center gap-1",
                                 e.quantity >= 0 ? "bg-[#E6FFF8] text-[#00B894]" : "bg-[#FFEDE9] text-[#E17055]"
                              )}>
                                 {e.quantity >= 0 ? <ArrowUpRight className="size-3" /> : <ArrowDownRight className="size-3" />}
                                 {MOVEMENT_TYPE_LABELS[e.type] || e.type.replace(/_/g, ' ')}
                              </Badge>
                           </div>
                        </td>
                        <td className="px-4 py-4 text-center">
         ...
         <div className="h-20 bg-[#F8F9FC] border-t border-[#E9ECF0] px-8 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
               <Button variant="outline" size="icon" disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="size-9 rounded-lg border-[#E9ECF0] hover:bg-white disabled:opacity-40"><ChevronLeft className="size-4 text-[#B2BEC3]" /></Button>
               <Button variant="outline" className="h-9 px-4 rounded-lg bg-[#6C5CE7] border-[#6C5CE7] text-white text-xs font-black">{page} / {totalPages}</Button>
               <Button variant="outline" size="icon" disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="size-9 rounded-lg border-[#E9ECF0] hover:bg-white disabled:opacity-40"><ChevronRight className="size-4 text-[#B2BEC3]" /></Button>
            </div>
         </div>
      </div>
    </div>
