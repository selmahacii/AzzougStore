...
export default function StockManager({ variant = 'all' }: { variant?: 'all' | 'alerts' | 'history' }) {
  ...
  return (
     <div className="space-y-6 animate-in fade-in duration-500">
                               ...
                               <td className="px-4 text-right">
                                  <div className="flex items-center justify-end gap-1">
                                     <button
                                        ...
                                        className="size-9 rounded-xl flex items-center justify-center text-[#B2BEC3] hover:text-[#0984E3] hover:bg-[#E8F4FE] transition-all"
                                     >
       ...
       {/* Product Detail Sheet ("Fiche produit") */}
       {viewingProduct && (
...
// ─── Product Detail Sheet — "Fiche produit" (section 2 du centre de
// pilotage) ─────────────────────────────────────────────────────────
// Montre uniquement des chiffres réellement disponibles en base (stock,
// réservé, coût, prix, historique) — pas de colonnes "en transit /
// endommagé / perdu" fantômes tant qu'aucun type de mouvement dédié
// n'existe pour les tracer (prévu à la phase Livreurs/Traçabilité).
function ProductDetailSheet({ product, storeId, onClose }: { product: any; storeId: string; onClose: () => void }) {
