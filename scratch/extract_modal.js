...
const effectMatch = adminContent.match(/(\/\/ Calculate shipping fees dynamically.*?\}\);)/s)[1];
const dialogMatch = adminContent.match(/(<Dialog open=\{isCreatingOrder\}.*?<\/Dialog>)/s);

let dialogCode = dialogMatch[1].replace(/isCreatingOrder/g, 'isOpen').replace(/setIsCreatingOrder/g, 'setIsOpen');
